...
import { Lock, Unlock, Shield, UserPlus, FilePlus2, Upload, Trash2, Key, RefreshCw, AlertTriangle, Plus, Check, Download, Database, UploadCloud, History } from "lucide-react";
...
export default function AdminPanel({
  ...
  React.useEffect(() => {
    try {
      ...
      console.error("Failed to load auto backups list:", e);
    }
  }, []);

  const downloadBackup = () => {
    const dataStr = "data:text/json;charset=utf-8," + encodeURIComponent(JSON.stringify(currentBackupObj, null, 2));
    const downloadAnchor = document.createElement('a');
    downloadAnchor.setAttribute("href", dataStr);
    const dateStr = new Date().toISOString().split('T')[0];
    downloadAnchor.setAttribute("download", `footworks_feed_backup_${dateStr}.json`);
    document.body.appendChild(downloadAnchor);
    downloadAnchor.click();
    downloadAnchor.remove();
  };

  const downloadSpecificBackup = (bk: any) => {
    const dataStr = "data:text/json;charset=utf-8," + encodeURIComponent(JSON.stringify(bk.data, null, 2));
    const downloadAnchor = document.createElement('a');
    downloadAnchor.setAttribute("href", dataStr);
    downloadAnchor.setAttribute("download", `footworks_feed_backup_${bk.date}.json`);
    document.body.appendChild(downloadAnchor);
    downloadAnchor.click();
    downloadAnchor.remove();
  };
    ...
    if (e.target.files && e.target.files[0]) {
      ...
      fileReader.onload = (event) => {
        try {
          ...
          if (!parsed.players || !parsed.tournaments || !parsed.matches) {
            setFileError("Invalid structure: JSON must contain players, tournaments, and matches.");
            setSelectedFileJson(null);
            return;
          }
          setSelectedFileJson(parsed);
  ...
  const restoreFromUploadedFile = async () => {
    if (!selectedFileJson) return;
    const confirmRestore = window.confirm(
      "Are you sure you want to load this backup feed file? It will OVERWRITE all current player directories, scoreboards, and past championships. This action is irreversible!"
    );
    if (confirmRestore) {
      try {
        await onRestoreBackup(selectedFileJson);
        alert("Tournament database successfully restored from backup feed file!");
        setSelectedFileJson(null);
        setFileName("");
        // Reload autoBackups
        const saved = localStorage.getItem("fw_auto_backups_list");
  ...
  const restoreFromAutoBackup = async (bk: any) => {
    const confirmRestore = window.confirm(
      `Are you sure you want to load the automatic daily backup taken on ${bk.date}? It will OVERWRITE all current data.`
    );
    if (confirmRestore) {
      try {
        await onRestoreBackup(bk.data);
        alert(`Database successfully restored to daily snapshot from ${bk.date}!`);
      } catch (err: any) {
        alert("Restoration failed: " + err.message);
      }
    }
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-4">
        ...
        <div className="lg:col-span-4 space-y-4">
          ...
          {/* Database Backup & Feed Control */}
          <div className="bg-white border border-slate-200 rounded-3xl p-6 space-y-4 shadow-sm">
            <div>
              <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest block">Ledger Sync & Safety</span>
              <h3 className="text-sm font-black text-slate-900 mt-1 flex items-center gap-1.5">
                ...
                Tournament Backups & Feed
              </h3>
            </div>
            <p className="text-[11px] text-slate-400 leading-relaxed font-medium">
              Create a downloadable JSON feed backup file or load/import an existing feed file to restore all tournament brackets, scores, and players.
            </p>

            <div className="space-y-3 pt-1">
              {/* 1. Download Backup Feed */}
              <button
                type="button"
              ...
              >
                <Download className="h-3.5 w-3.5 text-emerald-400" /> Download Current Feed File
              </button>

              {/* 2. Upload Feed file */}
              <div className="border-t border-slate-100 pt-3 space-y-2">
                <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest block">Load Feed File</label>
                <div className="flex flex-col gap-2">
                  <div className="relative border border-dashed border-slate-200 rounded-2xl p-4 text-center hover:bg-slate-50 transition-colors">
                    <input
                      type="file"
                      accept=".json"
                    ...
                    <div className="space-y-1 flex flex-col items-center">
                      ...
                      <p className="text-[10px] font-bold text-slate-500 mt-1">
                        {fileName ? `Selected: ${fileName}` : "Click or drag JSON backup feed here"}
                      </p>
                    </div>
                  </div>

                  {fileError && <p className="text-[9px] text-rose-600 font-bold uppercase">{fileError}</p>}

                  {selectedFileJson && (
                    <button
                      type="button"
                      onClick={restoreFromUploadedFile}
                      className="w-full py-2.5 bg-emerald-600 hover:bg-emerald-700 text-white font-black uppercase tracking-wider rounded-2xl text-[10px] flex items-center justify-center gap-1.5 shadow-sm transition-colors cursor-pointer"
                    >
                      <Check className="h-3.5 w-3.5" /> Restore from Selected Feed File
                    </button>
                  )}
                </div>
              </div>

              {/* 3. Automatic Daily Backups list */}
              <div className="border-t border-slate-100 pt-3 space-y-2">
                <div className="flex items-center justify-between">
                  <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest block flex items-center gap-1">
                    <History className="w-3 h-3 text-slate-500" /> Daily Snapshots
                  </label>
                  <span className="text-[8px] bg-emerald-50 text-emerald-800 font-extrabold px-1.5 py-0.5 rounded uppercase">Auto Feed</span>
                </div>
                
                {autoBackups.length === 0 ? (
                  <p className="text-[10px] text-slate-400 italic">No daily snapshots saved yet. They are taken automatically once a day.</p>
                ) : (
                  <div className="max-h-44 overflow-y-auto space-y-1.5 pr-1">
                    {autoBackups.map((bk: any) => (
                      <div key={bk.id} className="p-2 bg-slate-50 border border-slate-200/50 rounded-xl flex items-center justify-between text-[10px] font-mono hover:bg-slate-100/50 transition-colors">
                        <div className="flex flex-col">
                          <span className="font-bold text-slate-700">{bk.date}</span>
                          <span className="text-[8px] text-slate-400">{(bk.data?.players?.length || 0)} players • {(bk.data?.tournaments?.length || 0)} tourneys</span>
                        </div>
                        <div className="flex items-center gap-1">
                          <button
                            type="button"
                            title="Download JSON"
                            onClick={() => downloadSpecificBackup(bk)}
                          ...
                          <button
                            type="button"
                            onClick={() => restoreFromAutoBackup(bk)}
                            className="px-2 py-1 bg-emerald-100 hover:bg-emerald-200 text-emerald-800 text-[9px] font-black uppercase tracking-wider rounded-lg transition-colors cursor-pointer"
                          >
                            Load
                          </button>
                        </div>
